...
public class JmDNSImpl extends JmDNS
{
    ...
    public JmDNSImpl(InetAddress addr, String hostname) throws IOException
    {
        ...
        try
        {
            ...
            //init(addr.isLoopbackAddress() ? null : addr, hostname); // [PJYF Oct 14 2004] Why do we disallow the loopback address?
        	init(addr, hostname);
        }
        catch (IOException e)
        {
...
//    	Log.d(TAG, "start2");
        for (Iterator<ServiceInfo> iterator = serviceInfos.iterator(); iterator.hasNext();)
        {
            try
            {
//            	Log.d(TAG, "start3");
...
//    	Log.d(TAG, "5");
    ...
    void handleServiceResolved(ServiceInfoImpl info)
    {
        ...
        if (list != null)
        {
            ...
            final ArrayList<ServiceListener> listCopy = new ArrayList<ServiceListener>(list);
            for (Iterator<ServiceListener> iterator = listCopy.iterator(); iterator.hasNext();)
            {
    ...
    public void addServiceTypeListener(ServiceTypeListener listener) throws IOException
    {
        ...
        for (Iterator<String> iterator = serviceTypes.values().iterator(); iterator.hasNext();)
        {
    ...
    public void unregisterAllServices()
    {
        ...
        synchronized (this)
        {
            ...
            for (Iterator<ServiceInfo> iterator = services.values().iterator(); iterator.hasNext();)
            {
            	list.add((ServiceInfoImpl) services.values().iterator().next());
            }
            services.clear();
        }
        for (Iterator<ServiceInfoImpl> iterator = list.iterator(); iterator.hasNext();)
        {
    ...
    public void registerServiceType(String type)
    {
        ...
        if (serviceTypes.get(name) == null)
        {
            if ((type.indexOf("._mdns._udp.") < 0) && !type.endsWith(".in-addr.arpa."))
            {
                ...
                for (Iterator<ServiceTypeListener> iterator = list.iterator(); iterator.hasNext();)
                {
                    iterator.next().serviceTypeAdded(new ServiceEventImpl(this, type, null, null));
                }
            }
        }
    }
    ...
    public void updateRecord(long now, DNSRecord rec)
    {
        ...
        for (Iterator<DNSListener> iterator = listenerList.iterator(); iterator.hasNext();)
        {
            DNSListener listener = iterator.next();
    ...
    void handleResponse(DNSIncoming msg) throws IOException
    {
        ...
        for (Iterator<DNSRecord> i = msg.answers.iterator(); i.hasNext();)
        {
    ...
    void handleQuery(DNSIncoming in, InetAddress addr, int port) throws IOException
    {
        ...
        for (Iterator<DNSRecord> i = in.answers.iterator(); i.hasNext();)
        {
            DNSRecord answer = i.next();
            if ((answer.getType() == DNSConstants.TYPE_A) || (answer.getType() == DNSConstants.TYPE_AAAA))
            {
            	Log.d(TAG, String.format("someone asked for A or AAAA host=%s", answer.getName()));
    ...
    public void recover()
    {
        ...
        if (DNSState.CANCELED != getState())
        {
            ...
            { // Synchronize only if we are not already in process to prevent dead locks
                ...
                Collection<ServiceInfo> oldServiceInfos = new ArrayList<ServiceInfo>(getServices().values());
    ...
    public String toString()
    {
        ...
        if (services != null)
        {
            for (Iterator<String> k = services.keySet().iterator(); k.hasNext();)
            {
                Object key = k.next();
        ...
        {
            for (Iterator<String> k = serviceTypes.keySet().iterator(); k.hasNext();)
            {
                Object key = k.next();
        ...
        if (serviceCollectors != null)
        {
            ...
            {
                for (Iterator<String> k = serviceCollectors.keySet().iterator(); k.hasNext();)
                {
                    Object key = k.next();
    ...
    private static class ServiceCollector implements ServiceListener
    {
        ...
        public ServiceInfoImpl[] list()
        {
            synchronized (infos)
            {
                return infos.values().toArray(new ServiceInfoImpl[infos.size()]);
            }
        }
        ...
        public String toString()
        {
            ...
            synchronized (infos)
            {
                for (Iterator<String> k = infos.keySet().iterator(); k.hasNext();)
                {
                    Object key = k.next();
